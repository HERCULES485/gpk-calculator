...
test('АПК ситуации: десять ветвей ожидаемого состава, ситуация по умолчанию существует', () => {
  assert.deepEqual(
  ...
  // упрощённого производства и у судебного приказа: там якорь тоже вводится
  // напрямую, а не через уточняющие поля. У ветви execution_compensation
  // primary_field НЕТ: её якорь равноправен с дискриминатором окончания
  // производства и датой окончания, все три поля лежат в блоке исходных
  // данных — как у ветвей rulings и enforcement.
  assert.deepEqual(
...
test('АПК реестр сроков: 20 узлов из 23 — без двух узлов-событий и узла-окна', () => {
  assert.equal(CHAIN_NODE_IDS.length, 23);
  ...
  assert.equal(Object.keys(TERM_REGISTRY_APK).length, 20);
...
// Данные, поднимающие все 23 узла разом. Ветви дискриминаторов выбраны так,
...
test('АПК buildView: на полном наборе данных считаются все 23 узла, incomplete пуст', () => {
  const view = buildView(ALL_NODES_INPUTS_APK, { today: TODAY_APK });
  assert.equal(view.cards.length, 23);
...
test('АПК buildView: без данных все 23 узла уходит в incomplete, расчёт не вызывается', () => {
  const view = buildView({}, { today: TODAY_APK });
  assert.equal(view.incomplete.length, 23);
