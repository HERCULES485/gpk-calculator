...
test('АПК ситуации: девять ветвей ожидаемого состава, ситуация по умолчанию существует', () => {
  assert.deepEqual(
  ...
  // компенсации за нарушение права на судопроизводство в разумный срок и у
  // упрощённого производства: там якорь тоже вводится напрямую, а не через
  // уточняющие поля. У ветви execution_compensation primary_field НЕТ: её
  // якорь равноправен с дискриминатором окончания производства и датой
  // окончания, все три поля лежат в блоке исходных данных — как у ветвей
  // rulings и enforcement.
  assert.deepEqual(
...
test('АПК реестр сроков: 19 узлов из 22 — без двух узлов-событий и узла-окна', () => {
  assert.equal(CHAIN_NODE_IDS.length, 22);
  ...
  assert.equal(Object.keys(TERM_REGISTRY_APK).length, 19);
...
// Данные, поднимающие все 22 узла разом. Ветви дискриминаторов выбраны так,
...
test('АПК buildView: на полном наборе данных считаются все 22 узла, incomplete пуст', () => {
  const view = buildView(ALL_NODES_INPUTS_APK, { today: TODAY_APK });
  assert.equal(view.cards.length, 22);
...
test('АПК buildView: без данных все 22 узла уходит в incomplete, расчёт не вызывается', () => {
  const view = buildView({}, { today: TODAY_APK });
  assert.equal(view.incomplete.length, 22);
