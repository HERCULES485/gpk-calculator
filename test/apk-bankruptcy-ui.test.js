...
import { BANKRUPTCY_SITUATION_CATEGORIES } from '../apk/bankruptcy-categories.js';
...
// Полный набор входных данных на все шестьдесят пять узлов сразу. Даты — из
...
test('банкротство UI: каждый из шестидесяти трёх узлов закреплён ровно за одной ситуацией', () => {
  assert.equal(BANKRUPTCY_NODE_IDS.length, 65);
...
test('банкротство UI: пятьдесят две ветви ожидаемого состава, ситуация по умолчанию существует', () => {
  assert.deepEqual(
...
test('банкротство UI: полный набор данных — шестьдесят пять карточек, incomplete пуст', () => {
  const view = buildViewBankruptcy(FULL_INPUTS);
  assert.equal(view.cards.length, 65);
...
test('банкротство UI: пустой ввод — ни одной карточки, все шестьдесят пять узлов в incomplete', () => {
  const view = buildViewBankruptcy({});
  assert.deepEqual(view.cards, []);
  assert.equal(view.incomplete.length, 65);
...
test('банкротство UI: включение отчёта об оценке в ЕФРСБ — тоже показывает первый рабочий день', () => {
  const view = buildViewBankruptcy({
    appraisal_report_copy_received_date_apk: '2025-12-26',
  });
  const card = cardById(view, 'appraisal_report_registry_inclusion_apk');
  assert.ok(card);
  assert.equal(card.kind, 'term');
  assert.equal(card.unit, 'working_day');
  assert.ok(card.first_working_day, 'first_working_day должен быть на карточке');
...
test('банкротство UI: включение отчёта об оценке в ЕФРСБ — без данных узел в incomplete с подписью поля', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'appraisal_report_registry_inclusion_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
test('банкротство UI: два узла обжалования порядка продажи (предприятие/имущество) — обычные term-карточки, у каждого norm.primary на ч. 1 ст. 61', () => {
  const view = buildViewBankruptcy({
    enterprise_sale_procedure_approval_ruling_date_apk: '2025-03-11',
    property_sale_procedure_approval_ruling_date_apk: '2025-03-11',
  });
...
test('банкротство UI: два узла обжалования порядка продажи — без данных каждый узел в incomplete со своим полем', () => {
  const view = buildViewBankruptcy({});
  for (const [id, field] of [
    ['enterprise_sale_procedure_approval_appeal_apk', 'enterprise_sale_procedure_approval_ruling_date_apk'],
    ['property_sale_procedure_approval_appeal_apk', 'property_sale_procedure_approval_ruling_date_apk'],
  ]) {
    const node = incompleteById(view, id);
    assert.ok(node, `узел "${id}" не в incomplete`);
    assert.deepEqual(node.missing_inputs.map((f) => f.id), [field]);
...
test('банкротство UI: обжалование определения по заявлениям конкурсного управляющего о завершении — обычная term-карточка, norm.primary на ч. 1 ст. 61', () => {
  const view = buildViewBankruptcy({
    bankruptcy_completion_request_ruling_date_apk: '2025-03-11',
  });
  const card = cardById(view, 'bankruptcy_completion_request_ruling_appeal_apk');
  assert.ok(card, 'карточка обжалования определения по заявлениям конкурсного управляющего не появилась');
  assert.equal(card.kind, 'term');
  assert.equal(card.norm, 'ч. 1 ст. 61 ФЗ № 127-ФЗ');
});

test('банкротство UI: обжалование определения по заявлениям конкурсного управляющего о завершении — без данных узел в incomplete со своим полем', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'bankruptcy_completion_request_ruling_appeal_apk');
  assert.ok(node, 'узел обжалования определения по заявлениям конкурсного управляющего не в incomplete');
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
// --- 8. Стадия наблюдения: два новых узла (ст. 66 п. 2 абз. 2, ст. 68 п. 3) ---

test('банкротство UI: ответ на запрос временного управляющего — показывает первый рабочий день', () => {
  const view = buildViewBankruptcy({
    observation_information_request_received_date_apk: '2025-12-26',
  });
  const card = cardById(view, 'observation_information_request_response_apk');
  assert.equal(card.kind, 'term');
  assert.equal(card.unit, 'working_day');
  assert.ok(card.first_working_day, 'first_working_day должен быть на карточке');
  assert.equal(card.norm, 'ст. 66 п. 2 абз. 2 ФЗ № 127-ФЗ');
});

test('банкротство UI: ответ на запрос временного управляющего — без данных узел в incomplete с подписью поля', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'observation_information_request_response_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
test('банкротство UI: ответ на запрос временного управляющего закреплён за собственной одноузловой ветвью', () => {
  const situation = SITUATIONS_BANKRUPTCY.find(
    (s) => s.id === 'observation_information_request_response',
  );
  assert.ok(situation);
  assert.deepEqual(situation.nodes, ['observation_information_request_response_apk']);
  assert.equal(
    situation.primary_field,
    'observation_information_request_received_date_apk',
  );
});

test('банкротство UI: уведомление о введении наблюдения — показывает первый рабочий день', () => {
  const view = buildViewBankruptcy({
    observation_introduction_ruling_date_apk: '2025-12-26',
  });
  const card = cardById(view, 'observation_introduction_notification_apk');
  assert.equal(card.kind, 'term');
  assert.equal(card.unit, 'working_day');
  assert.ok(card.first_working_day, 'first_working_day должен быть на карточке');
  assert.equal(card.norm, 'ст. 68 п. 3 ФЗ № 127-ФЗ');
});

test('банкротство UI: уведомление о введении наблюдения — без данных узел в incomplete с подписью поля', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'observation_introduction_notification_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
test('банкротство UI: уведомление о введении наблюдения закреплено за собственной одноузловой ветвью, отдельной от ответа на запрос', () => {
  const situation = SITUATIONS_BANKRUPTCY.find(
    (s) => s.id === 'observation_introduction_notification',
  );
  assert.ok(situation);
  assert.deepEqual(situation.nodes, ['observation_introduction_notification_apk']);
  assert.equal(situation.primary_field, 'observation_introduction_ruling_date_apk');
  assert.notEqual(situation.id, 'observation_information_request_response');
});

// --- 9. Последствия открытия конкурсного производства (ст. 127-129) ----------

test('банкротство UI: обжалование определения об утверждении конкурсного управляющего — закреплено за собственной одноузловой ветвью', () => {
  const situation = SITUATIONS_BANKRUPTCY.find(
    (s) => s.id === 'bankruptcy_manager_appointment_appeal',
  );
  assert.ok(situation);
  assert.deepEqual(situation.nodes, ['bankruptcy_manager_appointment_appeal_apk']);
  assert.equal(situation.primary_field, 'bankruptcy_manager_appointment_ruling_date_apk');
});

test('банкротство UI: обжалование определения об утверждении конкурсного управляющего — без данных узел в incomplete с подписью поля', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'bankruptcy_manager_appointment_appeal_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
test('банкротство UI: три узла обязанностей конкурсного управляющего — одна ветвь, общее поле нарисовано один раз', () => {
  const situation = SITUATIONS_BANKRUPTCY.find((s) => s.id === 'bankruptcy_declaration');
  assert.ok(situation);
  ...
  assert.equal(situation.primary_field, 'bankruptcy_declaration_ruling_date_apk');

  const view = buildViewBankruptcy({});
  const ids = (id) => incompleteById(view, id).missing_inputs.map((f) => f.id);
  assert.deepEqual(ids('bankruptcy_information_publication_apk'), [
    'bankruptcy_declaration_ruling_date_apk',
  ]);
  assert.deepEqual(
    ids('bankruptcy_property_inventory_apk'),
    ids('bankruptcy_information_publication_apk'),
  );
  assert.deepEqual(
    ids('bankruptcy_employee_dismissal_notice_apk'),
    ids('bankruptcy_information_publication_apk'),
  );
});

test('банкротство UI: направление сведений для опубликования — показывает первый рабочий день', () => {
  const view = buildViewBankruptcy({
    bankruptcy_declaration_ruling_date_apk: '2025-12-26',
  });
  const card = cardById(view, 'bankruptcy_information_publication_apk');
  assert.equal(card.kind, 'term');
  assert.equal(card.unit, 'working_day');
  assert.ok(card.first_working_day, 'first_working_day должен быть на карточке');
  assert.equal(card.norm, 'п. 1 ст. 128 ФЗ № 127-ФЗ');
});

test('банкротство UI: принятие в ведение имущества и инвентаризация — обычная месячная карточка', () => {
  const view = buildViewBankruptcy({
    bankruptcy_declaration_ruling_date_apk: '2025-03-11',
  });
  const card = cardById(view, 'bankruptcy_property_inventory_apk');
  assert.equal(card.kind, 'term');
  assert.equal(card.deadline, '2025-06-11');
  assert.equal(card.norm, 'абз. 2 п. 2 ст. 129 ФЗ № 127-ФЗ');
});

test('банкротство UI: уведомление работников о предстоящем увольнении — обычная месячная карточка', () => {
  const view = buildViewBankruptcy({
    bankruptcy_declaration_ruling_date_apk: '2025-03-11',
  });
  const card = cardById(view, 'bankruptcy_employee_dismissal_notice_apk');
  assert.equal(card.kind, 'term');
  assert.equal(card.deadline, '2025-04-11');
  assert.equal(card.norm, 'п. 2 ст. 129 ФЗ № 127-ФЗ');
});

test('банкротство UI: включение результатов инвентаризации в ЕФРСБ — отдельная ветвь, другой якорь', () => {
  const situation = SITUATIONS_BANKRUPTCY.find(
    (s) => s.id === 'bankruptcy_inventory_results_registry',
  );
  assert.ok(situation);
  assert.deepEqual(situation.nodes, ['bankruptcy_inventory_results_registry_apk']);
  assert.equal(situation.primary_field, 'property_inventory_completion_date_apk');
  assert.notEqual(situation.primary_field, 'bankruptcy_declaration_ruling_date_apk');

  const view = buildViewBankruptcy({
    property_inventory_completion_date_apk: '2025-12-26',
  });
  const card = cardById(view, 'bankruptcy_inventory_results_registry_apk');
  assert.equal(card.kind, 'term');
  assert.equal(card.unit, 'working_day');
  assert.ok(card.first_working_day, 'first_working_day должен быть на карточке');
  assert.equal(card.norm, 'п. 2 ст. 129 ФЗ № 127-ФЗ');
...
// --- 10. Обжалование по институту мирового соглашения (ст. 163 п. 1; ст. 165
// п. 4) — девятая и десятая appeal-ветви домена по ч. 1 ст. 61, отнесены к
// категории «Мировое соглашение».

test('банкротство UI: обжалование определения о возобновлении производства по делу — закреплено за собственной одноузловой ветвью', () => {
  const situation = SITUATIONS_BANKRUPTCY.find(
    (s) => s.id === 'settlement_cancellation_resumption_appeal',
  );
  assert.ok(situation);
  assert.deepEqual(situation.nodes, ['settlement_cancellation_resumption_appeal_apk']);
  assert.equal(situation.primary_field, 'settlement_cancellation_resumption_ruling_date_apk');
});

test('банкротство UI: обжалование определения о возобновлении производства по делу — обычная term-карточка, norm.primary на ч. 1 ст. 61', () => {
  const view = buildViewBankruptcy({
    settlement_cancellation_resumption_ruling_date_apk: '2025-03-11',
  });
  const card = cardById(view, 'settlement_cancellation_resumption_appeal_apk');
  assert.equal(card.kind, 'term');
  assert.equal(card.norm, 'ч. 1 ст. 61 ФЗ № 127-ФЗ');
});

test('банкротство UI: обжалование определения о возобновлении производства по делу — без данных узел в incomplete с подписью поля', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'settlement_cancellation_resumption_appeal_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
test('банкротство UI: обжалование определения о расторжении мирового соглашения — закреплено за собственной одноузловой ветвью, отдельной от возобновления', () => {
  const situation = SITUATIONS_BANKRUPTCY.find((s) => s.id === 'settlement_termination_ruling_appeal');
  assert.ok(situation);
  assert.deepEqual(situation.nodes, ['settlement_termination_ruling_appeal_apk']);
  assert.equal(situation.primary_field, 'settlement_termination_ruling_date_apk');
  assert.notEqual(situation.id, 'settlement_cancellation_resumption_appeal');
});

test('банкротство UI: обжалование определения о расторжении мирового соглашения — обычная term-карточка, norm.primary на ч. 1 ст. 61', () => {
  const view = buildViewBankruptcy({
    settlement_termination_ruling_date_apk: '2025-03-11',
  });
  const card = cardById(view, 'settlement_termination_ruling_appeal_apk');
  assert.equal(card.kind, 'term');
  assert.equal(card.norm, 'ч. 1 ст. 61 ФЗ № 127-ФЗ');
});

test('банкротство UI: обжалование определения о расторжении мирового соглашения — без данных узел в incomplete с подписью поля', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'settlement_termination_ruling_appeal_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
test('банкротство UI: оба новых узла мирового соглашения отнесены к категории «Мировое соглашение»', () => {
  const category = BANKRUPTCY_SITUATION_CATEGORIES.find((c) => c.title === 'Мировое соглашение');
  assert.ok(category);
...
// --- 15. Ходатайство об уменьшении размера денежных средств, исключаемых из
// конкурсной массы (п. 2 ст. 213.27) — отдельная одноузловая ветвь.

test('банкротство UI: ходатайство об уменьшении размера денежных средств — закреплено за собственной одноузловой ветвью, отдельной от обжалования перечня имущества', () => {
  const situation = SITUATIONS_BANKRUPTCY.find((s) => s.id === 'property_exclusion_amount_dispute');
  assert.ok(situation);
  assert.deepEqual(situation.nodes, ['property_exclusion_amount_dispute_apk']);
  assert.equal(situation.primary_field, 'property_exclusion_amount_notice_published_date_apk');
  assert.notEqual(situation.id, 'property_exclusion_ruling_appeal');
});

test('банкротство UI: ходатайство об уменьшении размера денежных средств — working_day-карточка с первым рабочим днём', () => {
  const view = buildViewBankruptcy({
    property_exclusion_amount_notice_published_date_apk: '2025-12-26',
  });
  const card = cardById(view, 'property_exclusion_amount_dispute_apk');
  assert.equal(card.kind, 'term');
  assert.equal(card.unit, 'working_day');
  assert.ok(card.first_working_day, 'first_working_day должен быть на карточке');
  assert.equal(card.norm, 'п. 2 ст. 213.27 ФЗ № 127-ФЗ');
});

test('банкротство UI: ходатайство об уменьшении размера денежных средств — без данных узел в incomplete с подписью поля', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'property_exclusion_amount_dispute_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
test('банкротство UI: новый узел ходатайства об уменьшении размера денежных средств отнесён к категории «Банкротство гражданина»', () => {
  const category = BANKRUPTCY_SITUATION_CATEGORIES.find((c) => c.title === 'Банкротство гражданина');
  assert.ok(category);
  assert.ok(category.ids.includes('property_exclusion_amount_dispute'));
});

// Задача — оспаривание сделки должника: срок исковой давности (п. 1 ст. 61.9,
// ст. 61.2/61.3 ФЗ № 127-ФЗ; п. 2 ст. 181 ГК РФ). Ветвь без primary_field —
// оба поля обязательны одновременно, якорь считается как более позднее из
// них внутри compute-функции (см. комментарий к узлу в apk/bankruptcy.js).

test('банкротство UI: новая ветвь оспаривания сделки должника отнесена к категории «Общее — применимо на нескольких стадиях»', () => {
  const category = BANKRUPTCY_SITUATION_CATEGORIES.find(
    (c) => c.title === 'Общее — применимо на нескольких стадиях',
  );
  assert.ok(category);
  assert.ok(category.ids.includes('transaction_challenge_limitation'));
});

test('банкротство UI: оспаривание сделки должника — без одного из двух полей узел в incomplete с обоими недостающими полями по очереди', () => {
  const view = buildViewBankruptcy({
    transaction_challenge_manager_knew_date_apk: '2024-03-11',
  });
  const node = incompleteById(view, 'transaction_challenge_limitation_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
test('банкротство UI: оспаривание сделки должника — оба случая якоря («узнал раньше»/«узнал позже» утверждения управляющим) считаются одной и той же карточкой', () => {
  const laterKnew = buildViewBankruptcy({
    transaction_challenge_manager_knew_date_apk: '2024-03-11',
    transaction_challenge_manager_appointed_date_apk: '2023-01-10',
  });
  const laterAppointed = buildViewBankruptcy({
    transaction_challenge_manager_knew_date_apk: '2022-05-01',
    transaction_challenge_manager_appointed_date_apk: '2023-01-10',
  });
  const cardLaterKnew = cardById(laterKnew, 'transaction_challenge_limitation_apk');
  const cardLaterAppointed = cardById(laterAppointed, 'transaction_challenge_limitation_apk');
  assert.equal(cardLaterKnew.kind, 'term');
  assert.equal(cardLaterKnew.deadline, '2025-03-11');
  assert.equal(cardLaterAppointed.deadline, '2024-01-10');
  assert.equal(cardLaterKnew.norm, 'п. 2 ст. 181 ГК РФ');
});

// Задача — оспаривание сделки должника-гражданина: срок исковой давности
// (п. 2 ст. 213.32 ФЗ № 127-ФЗ; п. 2 ст. 181 ГК РФ). Отдельная ветвь от
// transaction_challenge_limitation выше (юрлица) — одно обязательное поле,
// обычный primary_field, категория «Банкротство гражданина».

test('банкротство UI: новая ветвь оспаривания сделки должника-гражданина отнесена к категории «Банкротство гражданина»', () => {
  const category = BANKRUPTCY_SITUATION_CATEGORIES.find(
    (c) => c.title === 'Банкротство гражданина',
  );
  assert.ok(category);
  assert.ok(category.ids.includes('transaction_challenge_limitation_citizen'));
});

test('банкротство UI: оспаривание сделки должника-гражданина — без поля узел в incomplete', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'transaction_challenge_limitation_citizen_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
test('банкротство UI: оспаривание сделки должника-гражданина — с полем карточка считается', () => {
  const view = buildViewBankruptcy({
    transaction_challenge_financial_manager_knew_date_apk: '2024-03-11',
  });
  const card = cardById(view, 'transaction_challenge_limitation_citizen_apk');
  assert.equal(card.kind, 'term');
  assert.equal(card.deadline, '2025-03-11');
  assert.equal(card.norm, 'п. 2 ст. 181 ГК РФ');
});

test('банкротство UI: ветвь должника-гражданина отдельная от ветви юрлица — разные id ветвей и узлов', () => {
  const citizenSituation = SITUATIONS_BANKRUPTCY.find(
    (s) => s.id === 'transaction_challenge_limitation_citizen',
  );
  const corporateSituation = SITUATIONS_BANKRUPTCY.find(
    (s) => s.id === 'transaction_challenge_limitation',
  );
...
// --- Остаток главы VII (ст. 132, 140, 144, 145, 146 ФЗ № 127-ФЗ) — конкурсное
// производство -----------------------------------------------------------------

test('банкротство UI: принятие изъятого из оборота имущества собственником — обычная term-карточка', () => {
  const view = buildViewBankruptcy({
    excluded_property_notice_received_date_apk: '2025-03-11',
  });
  const card = cardById(view, 'excluded_property_acceptance_apk');
  assert.ok(card);
  assert.equal(card.kind, 'term');
  assert.equal(card.norm, 'п. 2 ст. 132 ФЗ № 127-ФЗ');
});

test('банкротство UI: принятие изъятого из оборота имущества — без данных узел в incomplete с подписью поля', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'excluded_property_acceptance_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
test('банкротство UI: оплата по договору купли-продажи права требования — working_day-карточка, показывает первый рабочий день', () => {
  const view = buildViewBankruptcy({
    claim_sale_contract_date_apk: '2025-12-26',
  });
  const card = cardById(view, 'claim_sale_payment_apk');
  assert.ok(card);
  assert.equal(card.kind, 'term');
  assert.equal(card.unit, 'working_day');
  assert.ok(card.first_working_day, 'first_working_day должен быть на карточке');
  assert.equal(card.norm, 'абзац второй п. 2 ст. 140 ФЗ № 127-ФЗ');
});

test('банкротство UI: оплата по договору купли-продажи права требования — без данных узел в incomplete с подписью поля', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'claim_sale_payment_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
...
test('банкротство UI: обжалование освобождения и обжалование отстранения конкурсного управляющего — отдельные карточки со своими полями', () => {
  const view = buildViewBankruptcy({
    manager_release_ruling_date_apk: '2025-03-11',
    manager_removal_ruling_date_apk: '2025-04-11',
  });
  const releaseCard = cardById(view, 'manager_release_appeal_apk');
  const removalCard = cardById(view, 'manager_removal_appeal_apk');
  assert.ok(releaseCard);
  assert.ok(removalCard);
  assert.equal(releaseCard.kind, 'term');
  assert.equal(removalCard.kind, 'term');
  assert.equal(releaseCard.norm, 'ч. 1 ст. 61 ФЗ № 127-ФЗ');
  assert.equal(removalCard.norm, 'ч. 1 ст. 61 ФЗ № 127-ФЗ');
  // Разные якоря — разные дедлайны, не один и тот же узел под двумя именами.
  assert.notEqual(releaseCard.deadline, removalCard.deadline);
});

test('банкротство UI: обжалование освобождения/отстранения управляющего — без данных оба узла в incomplete со своими полями', () => {
  const view = buildViewBankruptcy({});
  const release = incompleteById(view, 'manager_release_appeal_apk');
  const removal = incompleteById(view, 'manager_removal_appeal_apk');
  assert.ok(release);
  assert.ok(removal);
  assert.deepEqual(
    release.missing_inputs.map((f) => f.id),
    ['manager_release_ruling_date_apk'],
  );
  assert.deepEqual(
    removal.missing_inputs.map((f) => f.id),
    ['manager_removal_ruling_date_apk'],
  );
  assert.ok(release.missing_inputs[0].label);
  assert.ok(removal.missing_inputs[0].label);
});

test('банкротство UI: созыв собрания кредиторов о переходе к внешнему управлению — обычная term-карточка', () => {
  const view = buildViewBankruptcy({
    solvency_restoration_circumstances_discovered_date_apk: '2025-03-11',
  });
  const card = cardById(view, 'creditors_meeting_external_management_transition_apk');
  assert.ok(card);
  assert.equal(card.kind, 'term');
  assert.equal(card.norm, 'п. 1 ст. 146 ФЗ № 127-ФЗ');
});

test('банкротство UI: созыв собрания кредиторов о переходе к внешнему управлению — без данных узел в incomplete с подписью поля', () => {
  const view = buildViewBankruptcy({});
  const node = incompleteById(view, 'creditors_meeting_external_management_transition_apk');
  assert.ok(node);
  assert.deepEqual(
    node.missing_inputs.map((f) => f.id),
