...
import { SITUATION_CATEGORIES_APK } from './categories.js';
...
function fieldOrPointer(id, labelOverride) {
  if (fieldAlreadyRendered(id)) {
    const label = labelOverride ?? INPUT_LABELS_APK[id];
...
function renderSituationSwitch(current) {
  ...
  root.textContent = '';
  const byId = new Map(SITUATIONS_APK.map((s) => [s.id, s]));
  ...
  for (const category of SITUATION_CATEGORIES_APK) {
