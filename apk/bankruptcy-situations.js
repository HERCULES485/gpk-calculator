...
export const SITUATIONS_BANKRUPTCY = [
  ...
  {
    id: 'bankruptcy_signs_registry_notification',
    label: 'Включение сведений о признаках банкротства в ЕФРСБ',
    // Единственное поле ветви — дата, когда руководителю должника стало
    // или должно было стать известно о признаках банкротства/обстоятельствах
    // ст. 8, 9 ФЗ № 127-ФЗ, тот же образец, что у остальных одноузловых
    // ветвей. Условия применимости (какие именно признаки/обстоятельства)
    // — текстом в logic узла, не отдельными полями (см. комментарий к узлу
    // в apk/bankruptcy.js).
    //
    // Первая по хронологии обязанность домена — возникает до возбуждения
    // дела о банкротстве, поэтому не относится ни к «Наблюдению», ни к
    // «Конкурсному производству»; отнесена к категории «Общее — применимо
    // на нескольких стадиях» вместе с остальными ЕФРСБ-узлами домена.
    primary_field: 'bankruptcy_signs_known_date_apk',
    fields: [],
    nodes: ['bankruptcy_signs_registry_notification_apk'],
  },
  {
    id: 'filing_notice_validity',
    label:
      'Утрата силы уведомления о намерении обратиться с заявлением о банкротстве (только юрлица)',
    // Единственное поле ветви — дата опубликования уведомления о намерении
    // обратиться с заявлением о банкротстве в ЕФРСБ. Применимость только к
    // банкротству юридических лиц описана текстом в logic/title узла (см.
    // комментарий к узлу в apk/bankruptcy.js), не отдельным полем-
    // переключателем.
    //
    // Отнесена к категории «Общее — применимо на нескольких стадиях» вместе
    // с остальными ЕФРСБ-узлами домена — институт предварительного
    // уведомления (глава I ФЗ № 127-ФЗ) существует до возбуждения дела о
    // банкротстве, отдельной стадии под него нет.
    primary_field: 'filing_notice_publication_date_apk',
    fields: [],
