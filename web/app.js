...
const INPUT_HINTS = {
  ...
  cassation_return_ruling_date:
    'Обжалуется в тот же кассационный суд, который вынес определение, — 1 месяц со дня ' +
  ...
  child_return_reasoned_decision_date:
    'Апелляция по такому делу — 10 рабочих дней со дня принятия решения в окончательной ' +
    'форме (ч. 1 ст. 244.17), а не месяц общего порядка',
  child_return_interim_ruling_date:
    'Частная жалоба на определение по такому делу — 10 рабочих дней со дня его вынесения ' +
    '(ч. 1 ст. 244.18), а не 15 дней общего порядка (ст. 332)',
  adoption_reasoned_decision_date:
    'Апелляция по такому делу — 10 рабочих дней со дня принятия решения в ' +
    'окончательной форме (ч. 2.1 ст. 274), а не месяц общего порядка. Норма ' +
...
const FOLLOW_UP_FIELDS = {
  ...
  // Даты апелляционного определения районного суда — у события вступления в силу
  // (это один документ): принятие определяет вступление в силу (ч. 1 ст. 209) и
  // предъявление ИЛ, изготовление мотивированного — точку отсчёта кассации.
  mirovoy_entry_into_force: {
    prompt: 'Дело прошло апелляцию в районном суде? Укажите даты апелляционного определения.',
    fields: [
...
function renderSituationFields(situation, primaryFilled) {
  ...
  if (situation.id === 'separate') {
    root.appendChild(
      el(
        'p',
        null,
        'Сроки, которые считаются независимо от цепочки обжалования и от категории ' +
          'дела: замечания на протокол и частная жалоба — в рабочих днях ' +
  ...
  if (situation.id === 'child_return') {
    root.appendChild(
      el(
        'p',
        null,
        'Дела о возвращении ребёнка и об осуществлении прав доступа на основании ' +
          'международного договора РФ (глава 22.2 ГПК): сроки обжалования здесь короче ' +
          'общего порядка — по десять рабочих дней и на апелляцию (ч. 1 ст. 244.17), и ' +
          'на частную жалобу (ч. 1 ст. 244.18). Даты независимы, заполните нужную.',
      ),
    );
  }
  if (situation.id === 'adoption') {
    root.appendChild(
      el(
        'p',
        null,
        'Дела об усыновлении (удочерении) ребёнка (глава 29 ГПК): апелляция на решение ' +
          'об удовлетворении заявления — десять рабочих дней со дня принятия решения в ' +
          'окончательной форме (ч. 2.1 ст. 274), а не месяц общего порядка. При отказе в ' +
          'удовлетворении заявления действует общий месячный срок (ст. 321).',
      ),
    );
  }
